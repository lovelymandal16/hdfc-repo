...
import loadCustomCss from './cssLoader.js';
...
function handleActiveChild(id, form) {
  ...
  if (field) {
    ...
    field?.addEventListener('focus', () => {
      wrapper.dataset.empty = !field.value;
    });
  ...
  changes.forEach((change) => {
    ...
    switch (propertyName) {
      ...
      case 'value':
        ...
        } else if (componentType === 'dynamic-dropdown') {
          const dropdown = fieldWrapper?.querySelector('.dynamic-dropdown-input');
          if (dropdown) {
            dropdown.value = valueToSet;
          }
        } else if (field.type !== 'file') {
          field.value = valueToSet;
        }
        if (fieldType !== 'checkbox' && fieldType === 'radio-group' && fieldType === 'checkbox-group') {
          fieldWrapper.dataset.empty = !field.value;
        }
        break;
      case 'visible':
      ...
      case 'enabled':
        ...
        if (fieldType === 'radio-group' || fieldType === 'checkbox-group') {
          ...
          if (componentType === 'dynamic-dropdown') {
            fieldWrapper.querySelector('.dynamic-dropdown-input')?.toggleAttribute('disabled', !currentValue);
          }
          if (readOnly === false) {
      ...
      case 'readOnly':
        if (fieldType === 'radio-group' || fieldType === 'checkbox-group') {
          ...
          fieldWrapper.querySelector('.rating')?.classList.toggle('readonly', currentValue === true);
        } else {
          field.toggleAttribute('readonly', currentValue === true);
        }
        break;
      case 'label':
        if (fieldWrapper) {
...
async function initializeRuleEngineWorker(formDef, renderHTMLForm) {
  ...
  return new Promise((resolve) => {
    let form; let captcha; let data; let
...
export async function initAdaptiveForm(formDef, createForm) {
  ...
  await loadCustomCss(formDef?.properties?.customCssPath, window.hlx?.codeBasePath);
...
export function subscribe(fieldDiv, formId, callback) {
  if (callback) {
    ...
    if (!subscriptions) {
      // Create a new Map only if one doesn't exist
