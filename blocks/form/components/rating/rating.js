...
export default function decorate(fieldDiv, fieldJson) {
  ...
  // Add appropriate class based on component state
  if (enabled === false) {
    ratingDiv.classList.add('disabled');
  } else if (readOnly === true) {
    ratingDiv.classList.add('readonly');
  }
  ...
  for (let i = 1; i <= max; i += 1) {
    ...
    star.addEventListener('click', () => {
      // Only process click if the component is not disabled or readonly
      if (!ratingDiv.classList.contains('disabled') && !ratingDiv.classList.contains('readonly')) {
    ...
    star.addEventListener('mouseover', () => {
      // Only process hover if the component is not disabled or readonly
      if (!ratingDiv.classList.contains('disabled') && !ratingDiv.classList.contains('readonly')) {
  ...
  ratingDiv.addEventListener('mouseleave', () => {
    // Only process mouseleave if the component is not disabled or readonly
    if (!ratingDiv.classList.contains('disabled') && !ratingDiv.classList.contains('readonly')) {
