...
export function createButton(fd) {
  ...
  button.textContent = fd?.label?.visible === false ? '' : fd?.label?.richText ? stripTags(fd?.label?.value) : stripTags(fd?.label?.value, '');
...
export function createDropdownUsingEnum(fd, wrapper) {
  ...
  // Only select placeholder if no option was explicitly selected and placeholder exists
  if (ph && optionSelected === false) {
    ph.setAttribute('selected', '');
  }

  // If no placeholder and no option was selected, ensure no option is selected by default
  if (!ph && optionSelected === false && wrapper.options.length > 0) {
    // Set the first option as not selected to prevent browser default selection
    wrapper.value = '';
  }
}
