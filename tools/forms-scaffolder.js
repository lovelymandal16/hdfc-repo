...
function findBaseComponentByName(baseTypeName) {
  ...
  let baseComponent = baseComponents.find((comp) => comp.name.toLowerCase() === baseTypeName.toLowerCase());

  // If no exact match, try partial match
  if (!baseComponent) {
    baseComponent = baseComponents.find((comp) => comp.name.toLowerCase().includes(baseTypeName.toLowerCase())
...
function updateComponentDefinition(componentName) {
  ...
  try {
    ...
    if (customGroup) {
      ...
      const existingEntry = customGroup.components.find((comp) => comp['...'] === newComponentEntry['...']);
...
async function scaffoldComponent(cliOptions = null) {
 ...
 ╚═╝  ╚═╝ ╚══════╝ ╚═╝     ╚═╝     ╚═╝       ╚═════╝  ╚═╝  ╚═╝ ╚═╝     ╚═╝ ╚══════╝
  ...
  const baseComponents = getBaseComponents();
  let componentName; let baseComponent; let customEventName; let
    propertyChanges;
